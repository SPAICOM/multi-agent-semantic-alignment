...
class FederatedLinearOptimizerSAE():
    ...
    def __init__(self,
        ...
        assert len(channel_matrix[0].shape) == 2, "The matrix must be 2 dimesional."
    ...
    def __F_step(self,
        ...
        self.F_k[idx] = torch.tensor(solve_sylvester(A.cpu().numpy(), B.cpu().numpy(), C.cpu().numpy()), device=self.device, dtype=self.dtype)
        return None
    ...
    def fit(self,
            n_agents: int,
            datamodule_list: list[torch.Tensor], # list of DataModules of both encoders and decoders coupled
        ...
        output = {k:datamodule_list[k].train_data.z_decoder for k in range(self.n_agents)}
        ...
        with torch.no_grad():
                ...
                output = [complex_compressed_tensor(o.T, device=self.device).H for k,o in output.items()]
                ...
                if iterations:
                    for _ in tqdm(range(iterations)):
                        for agent in range(self.n_agents):
                            ...
                            self.__G_step(idx=agent ,input=input, output=output[agent])
                            
                        for agent in range(self.n_agents):  
                            print("Initiliazing the F process;") 
                            self.__F_step(idx= agent, input=input, output=output[agent])
    ...
    def transform(self,
        ...
        with torch.no_grad():
            ...
            # Perform the prewhitening step
            input = a_inv_times_b(self.L_input, input - self.mean_input) #white signal 
    ...
    def eval(self,
             input: torch.Tensor,
             output: list[torch.Tensor]) -> float:
        ...
        for agent in range(self.n_agents):
            ...
            loss += torch.nn.functional.mse_loss(preds_k, output[agent], reduction='mean').item()
